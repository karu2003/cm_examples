...
void FCWT::daughter_wavelet_multiplication(float *input, float *output, float const *mother, float scale, int isize, bool imaginary, bool doublesided) {
    ...
    for (int q1 = 0; q1 < endpoint; q1++) {
        ...
        output[q1 * 2] = input[q1 * 2] * mother[static_cast<int>(tmp)];
        output[q1 * 2 + 1] = input[q1 * 2 + 1] * mother[static_cast<int>(tmp)] * (1 - 2 * imaginary);
    }
    ...
    if (doublesided) {
        for (int q1 = 0; q1 < endpoint; q1++) {
            ...
            output[(s1 - q1) * 2] = input[(s1 - q1) * 2] * mother[static_cast<int>(tmp)] * (1 - 2 * imaginary);
            output[(s1 - q1) * 2 + 1] = input[(s1 - q1) * 2 + 1] * mother[static_cast<int>(tmp)];
        }
    }
}
